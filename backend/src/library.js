  ...
  if (SONARR_API_KEY) {
    tasks.push((async () => {
      try {
        ...
        libraryCache.series = series.map(s => ({
      ...
      } catch (err) { console.error('Library cache - Sonarr:', err.message); }
    })());
  }
  ...
  if (RADARR_API_KEY) {
    tasks.push((async () => {
      try {
        ...
        libraryCache.movies = movies.map(m => ({
      ...
      } catch (err) { console.error('Library cache - Radarr:', err.message); }
    })());
  }
  ...
  if (LIDARR_API_KEY) {
    tasks.push((async () => {
      try {
        ...
        libraryCache.artists = artistList;
        try {
          ...
          allAlbums.forEach(a => {
            if ((a.statistics?.trackFileCount || 0) > 0)
              _dlByArtist[a.artistId] = (_dlByArtist[a.artistId] || 0) + 1;
          });
          artistList.forEach(a => { a.downloadedAlbumCount = _dlByArtist[a.id] || 0; });
          // Keep the lightweight library cache scoped to albums that already have files on disk.
          libraryCache.albums = allAlbums
        ...
        } catch (e) { console.error('Library cache - Lidarr albums:', e.message); }
      } catch (err) { console.error('Library cache - Lidarr:', err.message); }
    })());
  }

  await Promise.allSettled(tasks);
  libraryCache.lastRefresh = new Date().toISOString();
}
...
router.get('/library/refresh', async (req, res) => {
  ...
  res.json({ ok: true, lastRefresh: libraryCache.lastRefresh });
});
...
router.get('/lookup/music', async (req, res) => {
  ...
  try {
    ...
    res.json({ artists: [], albums: [], singles: [], topCategory: 'artists' });
  }
});
...
router.post('/add/music', async (req, res) => {
  ...
  try {
    ...
    const addPayload = {
      ...
      rootFolderPath: rootFolderPath || '/music',
      monitored: true,
      monitorNewItems: 'all',
      addOptions: {
