...
import { logServerEvent, metadataCache, summarizeError } from '../state.js';
...
async function lookupMediaInfo(hash) {
  ...
  const hashRef = summarizeHash(hash);

  try {
    const torrentResp = await qbFetchJson(`/api/v2/torrents/info?hashes=${hash}`);
    const torrents = Array.isArray(torrentResp) ? torrentResp : [];
    if (torrents.length === 0) {
      return { status: 'torrent_not_found' };
    }
    ...
    try {
      ...
      upstreamFailures.push({ source: 'radarr', error: summarizeError(err) });
    }

    if (!result && CONFIG.SONARR_API_KEY) {
      try {
        if (matchSources.length <= 2) {
          const contentResp = await qbFetchJson(`/api/v2/torrents/files?hash=${hash}`);
          ...
          if (match) {
            ...
            if (season != null) {
              try {
                ...
                logServerEvent('warn', 'media.lookup.episode_art_failed', {
                  service: 'sonarr',
                  hash: hashRef,
                  seriesId: match.id,
                  season,
        ...
        upstreamFailures.push({ source: 'sonarr', error: summarizeError(err) });
      }
    }

    if (result) {
      setMetadataCacheEntry(hash, result);
      return { status: 'ok', data: result, source: 'live' };
    }

    if (upstreamFailures.length > 0) {
      logServerEvent('error', 'media.lookup.upstream_failed', {
        hash: hashRef,
    ...
    logServerEvent('error', 'media.lookup.failed', {
      hash: hashRef,
      error: summarizeError(err),
    });
    return {
      status: 'upstream_error',
      failures: [{ source: 'qbittorrent', error: summarizeError(err) }],
    };
  }
}
...
router.get('/media-info/batch', async (req, res) => {
  try {
    ...
    await Promise.all(hashes.map(async (hash) => {
      const lookup = await lookupMediaInfo(hash);
...
router.get('/media-info/:hash', async (req, res) => {
  try {
    const lookup = await lookupMediaInfo(req.params.hash);
    if (lookup.status === 'upstream_error') {
      return res.status(502).json({
        error: 'Media lookup upstream failure',
    ...
    logServerEvent('error', 'media.lookup.route_failed', {
...
router.get('/poster', async (req, res) => {
  ...
  try {
    ...
    if (!isAllowedPosterHost(parsed.hostname)) {
      logServerEvent('warn', 'media.poster.host_rejected', {
    ...
    if (!resp.ok) {
      logServerEvent('error', 'media.poster.fetch_failed', {
        ...summarizeTarget(url),
        status: resp.status,
      });
    ...
    if (!contentType.startsWith('image/')) {
      logServerEvent('error', 'media.poster.non_image_response', {
        ...summarizeTarget(url),
        contentType,
      });
    ...
    const event = parsed ? 'media.poster.request_failed' : 'media.poster.invalid_url';
    logServerEvent(parsed ? 'error' : 'warn', event, {
      ...(parsed ? summarizeTarget(url) : { target: String(url) }),
      error: summarizeError(err),
    });
    res.status(parsed ? 502 : 400).json({ error: parsed ? 'Failed to fetch poster' : 'Invalid URL' });
  }
});
...
router.get('/arr-image/:service/*', async (req, res) => {
  ...
  try {
    ...
    if (!resp.ok) {
      logServerEvent('error', 'media.arr_image.fetch_failed', {
        service,
        path: basePath,
        status: resp.status,
      });
    ...
    const contentType = resp.headers.get('content-type') || 'image/jpeg';
    if (!contentType.startsWith('image/')) {
      logServerEvent('error', 'media.arr_image.non_image_response', {
        service,
        path: basePath,
        contentType,
      });
    ...
    logServerEvent('error', 'media.arr_image.request_failed', {
      service,
