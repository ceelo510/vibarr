...
function pruneFastSearchCache() {
  const now = Date.now();
  for (const [k, v] of fastSearchCache) {
    if (now - v.ts >= 5 * 60 * 1000) fastSearchCache.delete(k);
  }
}
setInterval(pruneFastSearchCache, 60000);
...
router.get('/manual-search', async (req, res) => {
  ...
  let aborted = false;
  // Stop shaping a large Arr response after the client disconnects.
  ...
  try {
    ...
    if (service === 'radarr' && RADARR_API_KEY) {
      ...
      const data = await fetchWithTimeout(url, 30000);
      if (aborted) return;
      releases = Array.isArray(data) ? data : [];
    } else {
