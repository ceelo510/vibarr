...
async function slskdFetch(path, options = {}) {
  ...
  // Bound polling calls so a stalled SLSKD request does not pin the interval loop.
...
const _slskdFetchInterval = setInterval(fetchSlskdDownloads, 10000);
registerInterval(_slskdFetchInterval);

const _slskdProcessInterval = setInterval(processCompletedSlskdDownloads, 30000);
registerInterval(_slskdProcessInterval);
